...
**Figure legend（図の説明文）**
詳細は後述しますが、論文の図表には文字通り図（figure）と表（table）があります。TableはWordで作成するのが通常です（Excelで作成してcopy & pasteします）。
一方で図はJPEG・TIFF・PNGなどの画像ファイルである必要があります。そのため、図の説明は画像内に入れるのではなく、本文の最後にfigure legends として挿入する必要があります。 
